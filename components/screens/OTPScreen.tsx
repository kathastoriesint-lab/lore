...
export default function OTPScreen() {
  const { navigate, pendingEmail, showToast } = useApp()
  ...
  const handleResend = useCallback(async () => {
    ...
    const { sendOTP } = await import('@/lib/game')
    try {
  ...
  // 6 visual digit boxes driven by the hidden input value
  ...
  return (
    <div className="otp-screen">
      ...
      <div className="otp-sub">
        Code sent to<br />
        <span className="otp-email">{pendingEmail || 'your email'}</span>
      </div>

      {/* Hidden real input — drives the visual boxes */}
      <div className="otp-boxes-wrap" onClick={() => inputRef.current?.focus()}>
        ...
        <div className="otp-boxes">
          {boxes.map((b, i) => (
            <div
              key={i}
              className={`otp-box${b.active ? ' active' : ''}${b.filled ? ' filled' : ''}`}
            >
      ...
      <div className="otp-timer">
        {seconds > 0
          ? <>Expires in {mins}:{secs} · <button className="otp-resend-btn" onClick={handleResend}>Resend</button></>
          : <button className="otp-resend-btn" onClick={handleResend}>Code expired — resend</button>
        }
      </div>

      <div className="otp-spacer" />
      <div className="otp-verify-hint">
        {loading ? 'Verifying…' : 'Enter all 6 digits to continue'}
      </div>
      <button
        className="login-cta"
        onClick={() => verify(otp)}
